<template>
  ...
  <div class="ow-grid mt-10">
    <wj-flex-grid :itemsSource="result2" headersVisibility="Column" :initialized="initializeGrid">
      ...
      <wj-flex-grid-column :header="'번호'" :binding="'field1'" :width="70" :isReadOnly="true">
        <wj-flex-grid-cell-template cellType="Cell" v-slot="cell">
          {{ cell.row.index + 1 }}
        </wj-flex-grid-cell-template>
      </wj-flex-grid-column>
      <wj-flex-grid-column :header="'시스템구분'" :binding="'field2'" :dataMap="systemlist" width="*" />
      <wj-flex-grid-column :header="'업무코드'" :binding="'field3'" :dataMap="bizlist" width="*" />
      <wj-flex-grid-column :header="'화면ID'" :binding="'field4'" width="*" />
      <wj-flex-grid-column :header="'화면명'" :binding="'field5'" width="*" />
      <wj-flex-grid-column :header="'화면URL'" :binding="'field6'" width="*" />
      <wj-flex-grid-column :header="'데이터권한'" :binding="'field7'" width="*" />
      <wj-flex-grid-column :header="'CRUD제어'" :binding="'field8'" width="*" />
      <wj-flex-grid-column :header="'대쉬보드'" :binding="'field9'" width="*" />
      <wj-flex-grid-column :header="'오늘의할일'" :binding="'field10'" width="*" />
      <wj-flex-grid-column :header="'화면비율'" :binding="'field11'" width="*" />
      <wj-flex-grid-column :header="'사용'" :binding="'field12'" width="*" />
    </wj-flex-grid>
  </div>
</template>
      ...
      result: [
        {
          items: [
            ...
            {
              header: '교육',
              items: [{ header: '' }, { header: '' }, { header: '' }, { header: '' }, { header: '' }],
            },
          ],
        },
      ],
